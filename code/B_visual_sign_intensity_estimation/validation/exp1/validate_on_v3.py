...
def validate(main_model_dir, path, labels_path, num_classes_input, bs, sz, k_folds, gt_mode, visual_signs):
    ...
    for n_fold in range(k_folds):
        ...
        multihead_efficientnet = Network(num_classes_input, device)
        learn = Learner(data, multihead_efficientnet, callback_fns=FMAPE)

        # Load and preprocess dataset
        learn, num_classes = LegitDataloader(learn, labels_path, visual_signs, label_max=3, max_value=num_classes_input-1, gt_mode=gt_mode)
        ...
        # Compute metrics
        class_list = [i for i in range(num_classes_input)]
        ...
        # All images are in the training set
        val_x = learn.data.train_ds.x.items
        val_y = learn.data.train_ds.y
...
if __name__ == '__main__':
    ...
    # Paths - Set the path to V2 dataset and use all images to test
    root_path = config['DATA']['DATASET_ROOT_PATH']
    root = root_path + 'LegitHealth-AD-FPK-IVI/'
    path = root + 'images/'
    ...
    for num_classes in num_classes_vect:
        for gt_mode in gt_modes:
            ...
            gt_mode = 'mean'
            results = validate(main_model_dir, path, labels_path, num_classes, bs, sz, k_folds, gt_mode, visual_signs)
